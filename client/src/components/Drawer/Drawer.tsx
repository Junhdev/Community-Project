...
import { FRIEND_REQUEST_ACTION } from '@/src/utils';
import { sendLogoutRequest } from '@/src/components/Friends/FriendsAPI';
import Modal from '@/src/common/Modal';
import * as S from '@/src/components/Drawer/Drawer.style';
import useInput from '../../hooks/useInput';

import axios from 'axios';
import { Friend } from '@/src/types';
import { myInfo } from '@/src/atoms/friendAtom';
...
const ProfileEditForm = ({ setIsProfileEditModalOpen }: { setIsProfileEditModalOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
  ...
  return (
    <S.ProfileEditForm onSubmit={postProfileChange}>
      <S.Profile>
        <S.ProfileImageForm>
          <img src={profileImg ? URL.createObjectURL(profileImg) : `/${myProfile!.profileImg}`} alt="profile-img" />
          <S.EditRound>
    ...
    <>
      {myProfile && (
        <S.ProfileSection>
          ...
          <S.Profile>
            <S.ProfileImage size="5rem" padding="1rem" src={'/' + myProfile.profileImg} />
            <S.ProfileInfo>
